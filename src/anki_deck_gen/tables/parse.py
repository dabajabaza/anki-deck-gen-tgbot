...
# Тире с пробелами (любое из трёх) или табуляция. Пробелы обязательны: иначе
# «crow's-feet» или «Санкт-Петербург» резались бы посередине.
...
def looks_like_text_table(text: str) -> bool:
    ...
    Не меньше двух непустых строк, не меньше двух из них с разделителем, и таких —
    хотя бы половина. Одна фраза с тире посреди — не колода; прозаический абзац с
    одним тире — тоже. А одна строка без разделителя среди нескольких с ним —
    Проблемная строка, которую человек поправит в диалоге, а не повод молчать.
    """
    lines = [line for line in text.splitlines() if line.strip()]
    if len(lines) < 2:
        return False
    with_separator = sum(1 for line in lines if TEXT_SEPARATOR.search(line))
    return with_separator >= 2 and with_separator * 2 >= len(lines)
