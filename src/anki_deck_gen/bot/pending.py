...
Здесь же копится текст, который человек шлёт строка за строкой: черновик — это
тот же Pending, просто ещё не завершённый (``draft``). Отдельного хранилища для
него нет по той же причине, по которой Pending один: два места для контекста
одного разговора неизбежно разойдутся.
...
class Pending:
    ...
    deadline: float = 0.0
    # Накопленные строки вставленного текста; None — Таблица пришла готовой
    # (файл, ссылка) или черновик уже завершён кнопкой «Готово» (A10).
    # Таблица и проверка пересобираются из этих строк на каждом сообщении.
